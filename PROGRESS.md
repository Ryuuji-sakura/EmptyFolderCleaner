...
- レビュー指摘のフェーズ3。①コントラスト改修（済）②進捗とキャンセル（済）
  ③行ごとの選択と「Finderで表示」（済）④破壊的ダイアログの既定ボタン（済）
  ⑤リサイズ対応・メニューバー整備（済）。英語ローカライズは見送り（日本語のみで提出）
...
## 完了項目（2026-09-24 / フェーズ3-④: 破壊的ダイアログの既定ボタン）

確認ダイアログで **Enter を押すと「完全に削除する」が実行されていた**。
`NSAlert` は最初に足したボタンを Return に割り当てるため、元に戻せない操作が
キー一撃で走る状態だった。完全削除モードでは既定ボタンを無くし、必ずクリックさせる。

Escape でキャンセルも効いていなかった。`NSAlert` が Escape を自動で割り当てるのは
ボタン名が英語の "Cancel" のときだけで、「キャンセル」には付かない。明示的に設定した。

1つのボタンに Return と Escape は同居できないので、より反射的に押される Escape の
ほうをキャンセルに残している（ゴミ箱モードは元に戻せるので Return は残す）。

UIテスト1件追加（計32ユニット + 7 UI = 39件）。Enter でダイアログが閉じないこと、
Escape で閉じて何も消えないことを、実際にキーを叩いて確かめている。

## 完了項目（2026-09-24 / フェーズ3-⑤: リサイズ・メニューバー）

### 英語ローカライズは見送り（ユーザー決定）

日本語のみで提出する。App Store の掲載言語も日本語のみになり、
ガイドライン2.3.7（名前の一貫性）も日本語名だけで揃う。後から追加はできる。

### ウィンドウがリサイズできるようになった

`.windowResizability(.contentSize)` と中身の `frame(width:height:)` の組み合わせで、
**ウィンドウは無言でリサイズ不可になっていた**。`.contentMinSize` に変え、中身は
最小サイズだけ持つようにした。広げた分は一覧が受け取る（4行しか見えなかったのが、
1280×800 で11行以上）。

App Store のスクリーンショットは 1280×800 以上が要るので、これが前提だった。
UIテストで実際に角をドラッグして 1320×840 まで広がることを確かめ、
**そのままPNGを書き出す**ようにしてある（掲載用の素材づくりに使える）。

リサイズして初めて分かった不具合も直した。ヘッダーの犬は `offset(x: 330)` の
固定座標に置いてあり、広げると画面の真ん中に取り残されてただの汚れに見えていた。
右端に寄せた。

### メニューバー

- 「フォルダ」メニュー: フォルダを選択（⌘O）/ もう一度調べる（⌘R）/ 中止（⌘.）
- 「新規」を削除。書類を作るアプリではないので、⌘N に反応して何も起きない項目だった
- **ヘルプを差し替えた**。既定の「空フォルダ削除ヘルプ」は存在しないヘルプブックを
  開こうとしてエラーダイアログになる。アプリ内の「使いかた」ウィンドウ（⌘?）にした

`openWindow` は `App` の本体からは読めないので、メニュー項目を View にして挟んでいる。

### パス表示をホーム基準に

対象フォルダのパスを `~/書類/…` と出すようにした。`/Users/名前/` の繰り返しが消えて、
肝心の末尾が読める。

`abbreviatingWithTildeInPath` は**サンドボックス内では効かない**。`NSHomeDirectory()`
がアプリのコンテナを指すため。実ユーザーのホームは `getpwuid(getuid())` から取っている。

### テスト

2件追加（計32ユニット + 9 UI = 41件、すべて成功）。

- ウィンドウが 1280×800 まで広がること（実際に角をドラッグする）
- 「フォルダ」メニューが存在し、ヘルプがアプリ内の説明に差し替わっていること

### 残っている作業（App Store提出まで）

- App Store Connect にアプリを登録（「アクセス制限なし」を選ぶところで止まっている）
...
## 完了項目（2026-09-24 / 走査中の帯グラフと行のダブルクリック）

### 帯グラフ

走査中は件数だけが出ていたが、「どのくらい進んだか」は分からなかった。

全体の件数は歩き終わるまで分からないので、割合は**目安**として出す。
渡されたフォルダの直下の項目を等分し、さらにその1つ下の階層で刻んで滑らかにする。
それより深いところでは値を据え置く（そこが何項目あるかは、やはり分からない）。

目安でしかないので**パーセントの数字は出さない**。正確なのは帯の下に出る実件数のほう。
削除中はこの限りではなく、何件消すか分かったうえで始めるので割合は正確。

分からないうちは `value` が nil になり、伸び縮みするだけの帯になる（SwiftUI の
`ProgressView(value:)` は optional を受け取り、nil で indeterminate になる）。

小さなスピナーは帯に置き換えた。

ユニットテスト2件追加。帯として嘘にならない条件 —— 0...1 に収まること、戻らないこと、
終わったら満ちること —— を固定している。

### 行のダブルクリックでFinderに表示

行の右端のボタンだけでは気づかれない。Finderと同じくダブルクリックでも開くようにした。

UIテストを1件追加したが、**最初は誤った判定で書いていた**。Finder が前面に来たかを
見ていたが、XCUITest は対象アプリを前面に保ち続けるため、「開いたのに前に出ていない」と
区別がつかない。実際に Finder のウィンドウが開いたかを見るように直した。
テスト後はそのウィンドウを閉じる（流すたびに Finder のウィンドウが増えるため）。

### ウィンドウのリサイズテストを安定させた

角を一度つかむだけでは、つかみ損ねや画面端で目標サイズに届かないことがあった
（1236pt で止まっていた）。実測して詰めるループに変え、移動もしてから広げるようにした。
1280×800 ちょうどに収まる。画面が狭い環境では `XCTSkipUnless` で飛ばす。


## 再開はここから（2026-09-25 時点）

### いまどこまで進んだか

- ビルド **1.5 (6)** は App Store Connect に**アップロード済み**
  （Transporter で 2026/09/25 1:01 デリバリ完了。処理も完了していて選択可能）
- サポートページとプライバシーポリシーは**公開済み**
- スクリーンショット（1280×800 × 2枚）は `docs/appstore/` にある
- ストアに貼る文面は全部 `docs/appstore-metadata.md` にある

### 止まっている一点

**App Store Connect のバージョンページで「保存」が通らない。**
見出しが「macOSアプリ バージョン 1.0」のままで、バージョン欄に 1.5 と打っても
保存されずに戻ってしまう。

画像はドロップした時点で即アップロードされるので残るが、テキスト欄は「保存」を
押して初めて確定する。保存に失敗すると打った内容が全部消える。

**未入力のまま保存を押しているのが原因と見ている。** 確認できている未入力:

- 「サインインが必要です」のチェックが入ったまま（ログイン機能は無いので外す）
- 連絡先情報の姓・名・電話番号が空（メールだけ入っている）

### 再開時の手順

長文を先に貼ると、保存に失敗したときにまた消える。軽いものだけ先に通す。

1. 「サインインが必要です」のチェックを外す
2. 連絡先情報の姓・名・電話番号を入力
3. バージョン欄を `1.5` に
4. 「保存」を押し、**保存ボタンが灰色になるまで待つ**（リロードしない）
5. 見出しが「バージョン 1.5」に変われば成功
6. `docs/appstore-metadata.md` から本文を貼って、もう一度保存
7. ビルド欄で `1.5 (6)` を選択
8. 左サイドバーの3ページを埋める
   - App Store → アプリのプライバシー → 「データを収集しません」
   - App Store → 価格および配信状況 → 無料、配信国
   - 一般 → アプリ情報 → 年齢制限（すべて「なし」＝4+）、カテゴリ「ユーティリティ」
9. 「審査用に追加」

保存時に赤いエラーが出たら、そこに本当の原因が書いてある。

### それでも 1.5 にできない場合

アプリ側を 1.0 に合わせる手がある。`project.yml` の `MARKETING_VERSION` を
`1.0`、`CURRENT_PROJECT_VERSION` を `7` にして `./Scripts/appstore.sh` で
作り直し、再アップロードする。ビルド番号は一度使うと再利用できないので 7 にする。
手数は増えるが確実。

### リリース方法の選択（未決）

いまは「自動でリリース」になっている。承認された瞬間に公開される。
初回は「手動でリリース」にしておくと、ストアページを確認してから公開でき、
GitHub 配布の終了とも足並みを揃えやすい。審査結果は変わらない。

### スクリーンショットの撮り直し方

UIテストで撮れる。ただし**テストランナーはサンドボックス内にいてデスクトップに
書き込めない**ので、デモ用フォルダの作成と後片付けはシェル側で行う。

1. シェルで `~/Desktop/空フォルダ削除デモ` を作る
   （写真/2023/沖縄、写真/2024/下書き、書類/請求書/2022、ダウンロード/古い資料、
   音楽/.DS_Store、書類/大事なメモ.txt。空フォルダが10件になる数が、
   一覧に収まりきって見栄えが良い）
2. 使い捨てのUIテストを足して、そのフォルダを `NSWorkspace.open` で開き、
   `resizeWindow(window, to: CGSize(width: 1280, height: 800))` で寸法を合わせ、
   `window.screenshot()` を書き出す
3. 撮ったらテストを消し、シェルでデモ用フォルダも消す

`resizeWindow` は UITests に残してある（実測して詰めるループ。角を一度つかむだけでは
届かないことがある）。
