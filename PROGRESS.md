...
v1.5（ビルド6）を Mac App Store の審査に提出した。Apple ID 6815618963。
...
| 配信 | 174か所（**中国本土のみ除外**） |
| 価格 | 無料 |
| リリース | 自動（承認された瞬間に公開）。審査中なら変更可 |
| 年齢制限 / カテゴリ | 4+ / ユーティリティ |
| プライバシー | データ収集なし + ポリシーURL |
| 掲載言語 | 日本語のみ |

中国本土を外したのは、ICP备案が必要になること、UIが日本語のみで実用性が薄いこと、
そして**配信国の変更は審査不要でいつでも戻せる**ため。いま含めて提出が止まる
リスクだけを避けた。

### 詰まったところ（次回のために）

**バージョンページの「保存」が通らず、見出しが 1.0 のままだった。**
原因は同じページの未入力項目だった。

- 「サインインが必要です」のチェックが入ったまま（ログイン機能は無いので外す）
- 連絡先情報の姓・名・電話番号が空（メールだけ入っていた）

**保存に失敗すると、そのとき打ち込んだ内容は全部消える。** 画像だけはドロップした
時点でアップロードされるので残り、それで「一部だけ保存された」ように見えて
混乱した。長文を貼る前に、軽い項目だけで一度保存を通すのが確実。

**プライバシーポリシーURLは、データを収集しないアプリでも必須。**
入力後に「公開」ボタンを押すまで確定しない。

### 指摘が来たときに効く材料

**Guideline 4.2（機能が最小限すぎる）** が最も可能性が高い。

- スキャン → 一覧で確認 → 行ごとに選択 → 実行、という工程がある
- 行のダブルクリック／右端のボタンで Finder に表示し、消す前に現物を確認できる
- ゴミ箱が既定。入れ子は1項目としてまとめて入るので「戻す」で原状復帰できる
- `.DS_Store` はマジックバイトを検査して本物だけを消す（名前だけでは判断しない）
- パッケージ・隠しフォルダ・シンボリックリンクを避ける判定を持つ
- 大きなフォルダでの進捗表示と中止

**Guideline 2.1（パフォーマンス）** は対策済み。ホームフォルダを渡されても件数と
帯グラフが出て、中止できる。App Review のメモにもその旨を書いてある。

### 承認されたあと

- GitHub の Releases 配布を終える（App Store 一本化の方針）
- README に App Store へのリンクを足す
